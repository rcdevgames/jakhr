  ...
  try {
    data.map((val) => {
      var object = {
        ...
        other_informations: objectOfother_informations(
          val.other_informations ?? null
        ),
      };
...
function objectOfpayrollModel(data = null) {
  ...
  try {
    ...
    objectData.value_to_reduce = objectOfvalue_to_reduce(
      data.value_to_reduce ?? null
    );
    objectData.other_informations = objectOfother_informations(
      data.other_informations ?? null
    );
  } catch (error) {
    console.log(error.message);
  }
