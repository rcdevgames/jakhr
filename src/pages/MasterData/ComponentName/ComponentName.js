...
const ComponentName = () => {
...

    {
      ...
      render: (val, record) => (
        <div className="btn-group" role="group">
          <a
            onClick={() => navigate(`${routes_name.M_COMPONENT_NAME_ALLOWANCE_SHOW}${val}`)}
          ...
          <a
            onClick={() => navigate(`${routes_name.M_COMPONENT_NAME_ALLOWANCE_DETAIL}${val}`)}
  ...
  const action = [
    <Link
      to={routes_name.M_COMPONENT_NAME_ALLOWANCE_CREATE}
  ...
  return (
    <AdminDashboard label="">
      <DataTablePagination
        fetchDataFunc={ providers.getDataAllowance
        }
        columns={columns}
        title={sys_labels.menus.ALLOWANCE}
        action={action}
      />
      <ActionModal
        onOk={handleDelete}
