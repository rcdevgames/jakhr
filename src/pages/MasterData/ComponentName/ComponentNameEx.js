...
// SysDateTransform
      ...
      render: (val, record) => (
        <div className="btn-group" role="group">
          <a
            onClick={() =>
              navigate(
                `${routes_name.M_COMPONENT_NAME_ALLOWANCE_EX_SHOW}${val}`
              )
            }
          ...
          <a
            onClick={() => navigate(`${routes_name.M_COMPONENT_NAME_ALLOWANCE_EX_DETAIL}${val}`)}
  ...
  const action = [
    <Link
      to={routes_name.M_COMPONENT_NAME_ALLOWANCE_EX_CREATE}
  ...
  return (
    <AdminDashboard label="">
      <DataTablePagination
        fetchDataFunc={providers.getDataAllowanceEx}
        columns={columns}
        title={sys_labels.menus.ALLOWANCE +' Lainnya'}
        action={action}
      />
      <ActionModal
        onOk={handleDelete}
...
export default ComponentNameEx;
