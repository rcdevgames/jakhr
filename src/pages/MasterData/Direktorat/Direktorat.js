...
const Organization = () => {
  ...
  const columns = [
    { title: "Direktorat", dataIndex: "name", key: "name", sortable: true },
    ...
    { title: "Tanggal Buat", dataIndex: "created_at", sortable: true, key: "created_at",render:(val,record)=>SysDateTransform({date:val,type:'long',checkIsToDay:true,lang:'in',withTime:true}) },
    {
      ...
      render: (val, record) => (
        <div className="btn-group" role="group">
          <a
            onClick={() => navigate(`${routes_name.M_DIREKTORI_SHOW}${val}`)}
          ...
          <a
            onClick={() => navigate(`${routes_name.M_DIREKTORI_DETAIL}${val}`)}
  ...
  const action = [
    <Link
      to={routes_name.M_DIREKTORI_CREATE}
  ...
  return (
    <AdminDashboard label="">
      <DataTablePagination
        ...
        columns={columns}
        title={sys_labels.menus.DIREKTORAT}
        action={action}
      />
      <ActionModal
        onOk={handleDelete}
